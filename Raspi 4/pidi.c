...
void write_read_buf(char* tbuf, char* rbuf, int len)
{
    ...
    while (rxcnt < len) // Tu je chyba, nikdy to nenastane a toci sa to dokola!, predpokladam ze nefunguje SPI
    {
        ...
        // rtapi_print_msg(RTAPI_MSG_INFO, "%s: Debug: rxcnt: %i, len: %i\n", modname, rxcnt, len);        
    }
...
int map_gpio()
{
    ...
    rtapi_print_msg(RTAPI_MSG_INFO, "DEBUG: peripheralsize: %08x\n", BLOCK_SIZE);
    rtapi_print_msg(RTAPI_MSG_INFO, "DEBUG: membase1: %08x\n", mem1_base);
    rtapi_print_msg(RTAPI_MSG_INFO, "DEBUG: membase2: %08x\n", mem2_base);
